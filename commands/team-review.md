...
>
> **Parameter bloat rule**: REJECT any constructor or function with more than 3 non-self parameters when a config/settings object exists that could hold them. If `__init__(self, title, docs_url, origins, enable_auth, enable_files, config)` exists and `config` already has fields for those values, the other params are bloat — they should be read from config. The constructor should be `__init__(self, config=None)`. Separate params are only justified when they genuinely vary per-call and don't belong in configuration.
...
> 9. **Server startup smoke test**: If the project has a backend server, there MUST be a test that boots the real server (or calls `on_startup`), then asserts ALL critical AppContext components are non-None (DAL, ModelManager, ServiceManager, FileManager). This catches the class of bug where integration tests bypass the server and test tasks directly, but the server itself fails to initialize a component — causing 503s in production while all tests pass. REJECT if no server startup test exists and the project has a server.
